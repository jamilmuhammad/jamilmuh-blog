...
const Card = ({ title, description, imgSrc, href, store, tech1, tech2, tech3 }) => {
  const eventName = `Click Redirect: Store ${href} - Project ${title}`
          ...
          <div className="flex flex-row items-center justify-between">
            ...
            <div className="flex flex-row justify-between">
              <div className="mx-1.5">
                {href ? (
                  <SocialIcon kind="external" href={href} size="6" eventName={eventName} />
                ) : null}
              </div>
              <div className="mx-1.5">
                {store ? (
                  <SocialIcon kind="store" href={store} size="6" eventName={eventName} />
                ) : null}
              </div>
            </div>
          </div>
