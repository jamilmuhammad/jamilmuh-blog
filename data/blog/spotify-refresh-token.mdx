...
I needed the Spotify Refresh Token for my blog site in which I could display my [Top 10 Tracks](https://jamilmuhammad-blog.vercel.app/stats) as well as display the currently playing track in the footer section.
...
- Add [Spotify link callback](https://getyourspotifyrefreshtoken.herokuapp.com/callback) to your `Redirect URIs` as
  shown in the image.
...
- You will be redirected to your redirect URI which in my case was set to *https://jamilmuhammad-blog.vercel.app/callback*.
...
- In the address bar you will find a huge URL string similar to the one below. In place of `$ACCESSCODE` there will be a long string of characters. Note down that string for the next step.
  ...
  ```bash
  https://jamilmuhammad-blog.vercel.app/callback?code=$ACCESSCODE
  ```
